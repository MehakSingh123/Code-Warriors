...
const db = mysql.createConnection({
    ...
    password: 'premsingh',  // Replace with your MySQL password
...
app.get('/results',isTeacher, (req, res) => {
...
app.post('/submit_form', (req, res) => {
  ...
  // Create an SQL query to insert the data into the database
  const query = 'INSERT INTO ' + tableName + ' (name, email,feed) VALUES (?, ?, ?)';
  const values = [name,email,feed];

  // Execute the SQL query to insert the data
  db.query(query, values, (err, result) => {
    if (err) {
      console.error('Error registering event:', err);
      res.send('Event registration not successful' );
    } else {
      // Registration successful
